package com.zy.zmail.server.north.Abudp.command;
 ...
 */
 public class AbudpCommand {
   ...
   public AbudpCommand(DoorMessage message){
       this.message = message;
   }

    protected void setStartAndEndFlag(byte[] data){
        data[index++]  = (byte) AbudpCommand.START_FLAG;
        data[index++] = (byte) AbudpCommand.START_REQUEST;

        data[data.length-2] = (byte) AbudpCommand.END_FLAG;
        data[data.length-1] = (byte) AbudpCommand.END_FLAG2;
    }
    ...
    public static AbudpCommand getInstance(DoorMessage message){
        return new AbudpCommand(message);
    }
}
